...
function App() {
	...
	*/
	...
	/*
	interface BigMSGProps {
		children: React.ReactNode;
		style?: React.CSSProperties;
	}
	function BigMSG({children, style }: BigMSGProps) {
		return (
		...
		<div className="App">
			...
			<div className="chessboard-wrapper"
				style={{
					...
					// transition: isTransitionEnabled ? 'transform 0.370s ease-in-out' : 'none',
					transition: isTransitionEnabled ? 'transform 0.75s ease-in-out' : 'none',
				}}
			>	
				{/* Ghost board behind actual game to prevent buggy animation */}
				{/*
				<div
					style={{
						display: 'none',
						position: 'absolute',
						top: '1.5rem',
						left: '0px', // dein Offset bleibt
						width: '31.25rem',
						height: '31.25rem',
						pointerEvents: 'none',
						zIndex: 2, // HÖHER als das normale Board
					}}
				>
					<Chessboard
				 		// animationDuration={0} // TODO - without: they move off the board due to bad orientation and no reload, with: theyare immediately visible
						position={game.fen()}
				...
				*/}
				<Chessboard
					key={isTransitionEnabled ? "turning" : "done"}
					position={game.fen()}
					onPieceDrop={onDrop}
